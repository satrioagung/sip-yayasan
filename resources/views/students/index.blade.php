...
<div class="space-y-5"
    ...
    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-4">
        <div class="flex flex-wrap items-center gap-3">
            ...
            {{-- Filter Kelas --}}
            <div class="relative"
                x-data="{ open: false, selected: '{{ request('class_id') }}', label: '{{ request('class_id') ? $kelas->firstWhere('id', request('class_id'))?->nama_kelas : 'Semua Kelas' }}' }"
                @click.outside="open = false">
                <button type="button"
                    @click="open = !open"
                    class="flex items-center gap-2 text-sm border border-gray-200 rounded-xl pl-3 pr-3 py-2.5
                ...
                <div x-show="open" x-transition
                    ...
                    <button type="button"
                        @click="selected = ''; label = 'Semua Kelas'; open = false; $nextTick(() => doSearch())"
                        class="w-full text-left px-4 py-2 hover:bg-gray-50 text-gray-700">
                        Semua Kelas
                    </button>
                    @foreach($kelas as $k)
                        <button type="button"
                            @click="selected = '{{ $k->id }}'; label = '{{ $k->nama_kelas }}'; open = false; $nextTick(() => doSearch())"
                            class="w-full text-left px-4 py-2 hover:bg-gray-50 text-gray-700">
                            {{ $k->nama_kelas }}
                        </button>
                    @endforeach
                </div>
                <input type="hidden" id="filter-kelas" :value="selected">
            </div>

            {{-- Filter Status --}}
            <div class="relative"
                x-data="{ open: false, selected: '{{ request('status') }}', label: '{{ request('status') ? ucfirst(request('status')) : 'Semua Status' }}' }"
                @click.outside="open = false">
                <button type="button"
                    @click="open = !open"
                    class="flex items-center gap-2 text-sm border border-gray-200 rounded-xl pl-3 pr-3 py-2.5
                ...
                <div x-show="open" x-transition
                    class="absolute z-20 mt-1 min-w-full bg-white border border-gray-200 rounded-xl shadow-lg py-1 text-sm">
                    <button type="button"
                        @click="selected = ''; label = 'Semua Status'; open = false; $nextTick(() => doSearch())"
                        class="w-full text-left px-4 py-2 hover:bg-gray-50 text-gray-700">
                        Semua Status
                    </button>
                    <button type="button"
                        @click="selected = 'aktif'; label = 'Aktif'; open = false; $nextTick(() => doSearch())"
                        class="w-full text-left px-4 py-2 hover:bg-gray-50 text-gray-700">
                        Aktif
                    </button>
                    <button type="button"
                        @click="selected = 'nonaktif'; label = 'Nonaktif'; open = false; $nextTick(() => doSearch())"
                        class="w-full text-left px-4 py-2 hover:bg-gray-50 text-gray-700">
                        Nonaktif
                    </button>
                </div>
                <input type="hidden" id="filter-status" :value="selected">
            </div>
